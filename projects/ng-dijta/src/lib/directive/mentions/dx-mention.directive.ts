...
import { getCaretPosition, getValue, insertValue, setCaretPosition } from './dx-mention-utils';

import { MentionConfig } from "./dx-mention-config";
...
export class MentionDirective implements OnChanges {
  ...
  private activeConfig!: MentionConfig | any;
  ...
  private DEFAULT_CONFIG: MentionConfig = {
    ...
    mentionSelect: (item: any, triggerChar?: string) => {
      return this.activeConfig.triggerChar + item[this.activeConfig.labelKey as string];
    },
    mentionFilter: (searchString: string, items: any[]) => {
      const searchStringLowerCase = searchString.toLowerCase();
      return items.filter(e => e[this.activeConfig.labelKey as string].toLowerCase().startsWith(searchStringLowerCase));
    }
  }
  ...
  private searchString!: string | any;
  ...
  private addConfig(config: MentionConfig) {
    ...
    if (items && items.length > 0) {
      ...
      if (typeof items[0] == 'string') {
        items = items.map((label) => {
          let object = {};
          object[config.labelKey as string] = label;
          return object;
        });
      }
      if (config.labelKey) {
        // remove items without an labelKey (as it's required to filter the list)
        items = items.filter(e => e[config.labelKey as string]);
        if (!config.disableSort) {
          items.sort((a, b) => a[config.labelKey as string].localeCompare(b[config.labelKey as string]));
        }
      }
    }
    config.items = items;
    ...
    // add the config
    this.triggerChars[config.triggerChar as string] = config;
  ...
  keyHandler(event: any, nativeElement: HTMLInputElement = this._element.nativeElement): any {
    ...
    let val: string | any = getValue(nativeElement);
  ...
  public startSearch(triggerChar?: string, nativeElement: HTMLInputElement = this._element.nativeElement) {
    triggerChar = (triggerChar || this.mentionConfig.triggerChar || this.DEFAULT_CONFIG.triggerChar) as string;
    const pos = getCaretPosition(nativeElement, this.iframe) as number;
  ...
  updateSearchList() {
    ...
    if (this.searchList) {
      this.searchList.items = matches as [];
  ...
  showSearchList(nativeElement: HTMLInputElement) {
    ...
    this.searchList.styleOff = this.mentionConfig.disableStyle as boolean;
