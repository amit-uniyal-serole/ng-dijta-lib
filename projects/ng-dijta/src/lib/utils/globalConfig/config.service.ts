...
import { dxGlobalConfig, dxGlobalConfigKey, dxGlobalConfigToken, dxGlobalInsideConfigKey } from './config';
...
export class DevConfigService {
  private configUpdated$ = new Subject<keyof dxGlobalConfig>();
  private config: dxGlobalConfig;
  constructor(@Optional() @Inject(dxGlobalConfigToken) defaultConfig?: dxGlobalConfig) {
    this.config = defaultConfig || {};
  }
  getConfigForComponent<T extends dxGlobalConfigKey>(componentName: T): dxGlobalConfig[T] {
    return this.config[componentName];
  }
  getConfigForApi<T extends dxGlobalInsideConfigKey>(api: any): dxGlobalConfig['global'][T] {
    const globalConfig = this.config['global'] || {};
    const apiConfig = globalConfig[api];
    return apiConfig;
  }
  getConfigChangeEventForComponent(componentName: dxGlobalConfigKey): Observable<void> {
    return this.configUpdated$.pipe(
      filter((n) => n === componentName),
      mapTo(undefined)
    );
  }
  set<T extends dxGlobalConfigKey>(componentName: T, value: dxGlobalConfig[T]): void {
