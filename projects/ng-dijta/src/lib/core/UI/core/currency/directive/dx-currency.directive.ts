import { Directive, ElementRef, HostListener, forwardRef, Renderer2, Input, Injector, OnInit, Inject, LOCALE_ID } from '@angular/core';
import { DecimalPipe, getLocaleCurrencyCode } from '@angular/common';
import {
...
@Directive({
    selector:
        'input[formControlName][dxCurrency], input[formControl][dxCurrency], input[ngModel][dxCurrency]',
...
export class CurrencyInputDirective implements ControlValueAccessor, Validator, OnInit {
    ...
    handleInput(rawInputValue: string, inputEl: HTMLInputElement, event: InputEvent) {
        ...
        if (this.hasNoMoreThan2DecimalDigits(rawInputValue) && !this.isNumberTooLarge(rawInputValue) && this.isCurrency) {
    ...
    private reformatInput(value?: number): void {
        if (this.isCurrency) {
            ...
            if (this.isCurrency) {
                ...
                this.setNativeInputValue(this.ngControl.value);
            }
        }
    ...
    private getCurrency(value: number): string {
        const currencyCd: string = getLocaleCurrencyCode(this.locale) ?? 'USD';
        return new Intl.NumberFormat(`${this.locale}`, { style: 'currency', currency: `${currencyCd}` }).format(Number(value))
    }
}
