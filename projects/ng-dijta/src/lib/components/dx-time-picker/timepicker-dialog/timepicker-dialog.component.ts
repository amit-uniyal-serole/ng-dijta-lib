import { Component, DoCheck, EventEmitter, Inject, Output, TemplateRef } from '@angular/core';
import { MAT_DIALOG_DATA } from '@angular/material/dialog';
import { ClockMode, ClockViewType, IAllowed12HourMap, IAllowed24HourMap } from '../interfaces-and-types';
import { MatTimepickerButtonTemplateContext } from '../timepicker.directive';
import { convertHoursForMode, twoDigits } from '../util';
...
export class MatTimepickerComponentDialogComponent implements DoCheck {
  ...
  set value(value: any) {
    value = value || this.minDate || this.maxDate || moment().set({
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data) {
    ...
    if(data.isPm === undefined) {
      const { isPm } = convertHoursForMode((new Date()).getHours(), this.mode);
      this.isPm = isPm;
    }
