...
export class MatTimepickerDirective
  ...
  private allowed24HourMap!: IAllowed24HourMap;
  private allowed12HourMap!: IAllowed12HourMap;
  ...
  private modalRef!: MatDialogRef<MatTimepickerComponentDialogComponent> | undefined;;
  ...
  ngOnInit() {
    if (this.ngControl && this.ngControl.control?.parent) {
      const [key]: any = Object.entries(this.ngControl.control.parent.controls).find(([, c]) => c === this.ngControl.control);
  ...
  writeValue(value: Date | any, isInnerCall = false): void {
    if (!isInnerCall) {
