...
import { backdropFadeInOut } from '../../core/animation/backdrop-fade-in-out';
import { wipeInOutAnimation } from '../../core/animation/wipe-in-out';
...
export class ModalComponent implements OnInit, OnDestroy {
  ...
  @Input() backdropCloseable: boolean | undefined;
  @Input()
  ...
  @Input()
  escapable!: boolean; // 是否支持esc键关闭弹窗
  @Input()
  cssClass!: string;
  ...
  pressEscToClose: Subscription | null = new Subscription();
  ...
  ngOnInit() {
    if (this.escapable && this.pressEscToClose) {
  ...
  show() {
    if (this.document.documentElement.scrollHeight > this.document.documentElement.clientHeight) {
      ...
      this.renderer.addClass(this.document.body, 'dx-body-scrollblock');
    ...
    if (!this.bodyScrollable && this.documentOverFlow) {
      this.renderer.addClass(this.document.body, 'dx-body-overflow-hidden');
    }
