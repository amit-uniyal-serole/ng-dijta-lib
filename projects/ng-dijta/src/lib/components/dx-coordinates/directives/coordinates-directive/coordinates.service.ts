...
export class CoordinatesService {
  transform(value: string | number, transformationType: TransformationType, direction?: Direction): string
    | number {

    if (!transformationType || !this.isValueValid(value, transformationType, direction)) {
      return ''
  ...
  isValueValid(value: string | number | null, transformationType: TransformationType, direction?: Direction): boolean {
    return (!value && value === 0) || (transformationType === 2 ? this.isValidDigit(value, direction) : this.isValidDegree(value, direction))
  }
  ...
  isValidDigit(value: string | number | null, direction?: Direction): boolean {
    ...
    if (typeof value === 'number' && !Number.isNaN(value)) {
      val = value
    } else if (typeof value === 'string' && !this.isValidDecimalFormat(value.toString())) {
      return false
    ...
    const boundaries = this.getDirectionBoundaries(direction)
    return !(Number(value) < boundaries[0] || Number(value) > boundaries[1])
  }

  private isValidDecimalFormat(value: string): boolean {
