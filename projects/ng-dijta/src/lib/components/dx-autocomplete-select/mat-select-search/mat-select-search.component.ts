...
export class MatSelectSearchComponent implements OnInit, OnDestroy, ControlValueAccessor {
  ...
  public _options$: BehaviorSubject<QueryList<MatOption> | any> = new BehaviorSubject<QueryList<MatOption> | any>(undefined);

  private optionsList$: Observable<MatOption[] | any> = this._options$.pipe(
    switchMap(_options => _options ?
      _options.changes.pipe(
        map((options: any) => options.toArray()),
  ...
  public _showNoEntriesFound$: Observable<boolean | string | null> = combineLatest([
