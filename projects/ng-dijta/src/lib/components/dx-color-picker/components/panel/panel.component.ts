...
import { DxColorsColor } from "../../clases/color";

@Component({
    selector: "dx-colors-panel",
...
export class PanelComponent implements OnInit {
    ...
    public hsva: Hsva | null = new Hsva(0, 1, 1, 1);
    ...
    private triggerInstance!: DxColorsTriggerDirective;
    private TriggerBBox;
    ...
    private findIndexSelectedColor(colors): number | undefined {
    ...
    public iniciate(
        triggerInstance: DxColorsTriggerDirective,
        triggerElementRef,
        color,
        palette,
        animation,
    ...
    private ChangeAlphaOnPalette(
        alpha,
        colors: Array<string | DxColorsColor>
    ): Array<any> {
        var result: any[] = [];
        for (let i = 0; i < colors.length; i++) {
            const color = colors[i];
            if (typeof color == "string") {
                ...
                let newColor = new DxColorsColor();
    ...
    setPreviewColor(value: Hsva | null) {
    ...
    isOutside(event) {
        return event.target.classList.contains("dx-colors-overlay");
    }
}