...
export class DImagePreviewComponent implements OnInit, OnDestroy {
    ...
    get targetImageSrc(): string | null {
    ...
    click($event) {
        if ($event.target.classList.contains('dx-image-preview-wrapper')) {
            this.onClose();
        }
    }
    ...
    private addFullScreenStyle() {
        this.document.querySelector('body')?.classList.add('dx-fullscreen');
    }

    private removeFullScreenStyle() {
        this.document.querySelector('body')?.classList.remove('dx-fullscreen');
    }

    private getImgElement(): HTMLElement {
        return this.elementRef.nativeElement.querySelector('.dx-image-preview-main-image');
    }
}