...
export class TreeNode implements ITreeNode {
  ...
  @computed get isAllSelected(): any {
    if (this.isSelectable()) {
      return this.treeModel.isSelected(this);
    } else {
  ...
  @observable children: TreeNode[] | any;
  ...
  @observable hideExpansionIcon!: boolean;
  ...
  constructor(public data: any, public parent: TreeNode | any, public treeModel: TreeModel, index: number) {
    ...
    this.index = index;
    this.icon = data?.icon;
    this.iconColor = data?.iconColor ?? '#354FBB';
    this.hideExpansionIcon = data?.hideExpansionIcon
  ...
  get realParent(): TreeNode | null { return this.isRoot ? null : this.parent; }

  // proxy functions:
  get options(): TreeOptions | any { return this.treeModel.options; }
  fireEvent(event: any) { this.treeModel.fireEvent(event); }
  ...
  // traversing:
  _findAdjacentSibling(steps: any, skipHidden = false) {
  ...
  @computed get visibleChildren() {
    return (this.children || []).filter((node: any) => !node.isHidden);
  }
  ...
  findNextNode(goInside = true, skipHidden = false): any {
  ...
  doForAll(fn: (node: ITreeNode) => any) {
    Promise.resolve(fn(this)).then(() => {
      if (this.children) {
        this.children.forEach((child: any) => child.doForAll(fn));
      }
    });
  }
  ...
  dispose() {
    if (this.children) {
      this.children.forEach((child: any) => child.dispose());
    }
    if (this.handler) {
      this.handler();
    }
  ...
  setIsActive(value: any, multi = false) {
  ...
  @action setIsSelected(value: any) {
    if (this.isSelectable()) {
      this.treeModel.setSelectedNode(this, value);
    } else {
      this.visibleChildren.forEach((child: any) => child.setIsSelected(value));
    }
  ...
  mouseAction(actionName: string, $event: any, data: any = null) {
  ...
  @action _initChildren() {
    this.children = this.getField('children')
      .map((c: any, index: any) => new TreeNode(c, this, this.treeModel, index));
  }
}
