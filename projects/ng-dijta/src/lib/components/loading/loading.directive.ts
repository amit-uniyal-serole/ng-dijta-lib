...
export class LoadingDirective implements OnChanges {
  ...
  active = true;
  backdropRef!: ComponentRef<any> | any;
  loadingRef!: ComponentRef<any> | null;
  ...
  loadingChangeEvent(loading: LoadingType | any): void {
    if (loading instanceof Subscription) {
      this.startLoading();
      loading.add(() => this.endLoading());
      return;
    }
    const loadingArr = [].concat(loading).map((item: any) => (item instanceof Observable ? item : from(item)));
  ...
  private startLoading(): void {
    ...
    if (!this.loadingRef) {
      this.loadingRef = this.viewContainerRef.createComponent(
        LoadingComponent, { index: undefined, injector: this.injector }
      );
  ...
  private createLoadingBackdrop(): void {
    this.backdropRef =
      ...
      this.viewContainerRef.createComponent(
        LoadingBackdropComponent, { index: undefined, injector: this.injector }
      );
    if (this.backdropRef)
