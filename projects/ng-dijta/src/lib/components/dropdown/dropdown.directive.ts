...
export class DropDownDirective implements OnDestroy, OnChanges, AfterContentInit {
  ...
  @HostBinding('class.dx-dropdown-open')
  ...
  @HostBinding('class.dx-dropdown') addClass = true;
  @Input() disabled = false;
  @HostBinding('class.dx-dropdown-animation')
  ...
  public cdkConnectedOverlayOrigin!: CdkOverlayOrigin | undefined;
  ...
  handleHoverSubscriptionIfTriggerIsHover() {
    if (this.trigger === 'hover') {
      const states: Observable<boolean> = merge(
        fromEvent<any>(this.el.nativeElement, 'mouseenter').pipe(mapTo(true)),
        fromEvent<any>(this.el.nativeElement, 'mouseleave').pipe(
          delay(100),
