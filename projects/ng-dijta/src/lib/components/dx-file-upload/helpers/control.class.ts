...
export class FileUploadControl {
    ...
    private errors: Array<{ [key: string]: any }> | (ValidationErrors | null)[] = [];
     ...
     */
    public readonly valueChanges: BehaviorSubject<Array<File> | any> = new BehaviorSubject([]);
    ...
    public getError(): Array<ValidationErrors> | (ValidationErrors | null)[] {
        return this.errors;
    }
    ...
    private analyzeToDiscard(): void {
        ...
        while (validators.length) {
            const validator: ValidatorFn | undefined = validators.shift();
            if (validator) {
