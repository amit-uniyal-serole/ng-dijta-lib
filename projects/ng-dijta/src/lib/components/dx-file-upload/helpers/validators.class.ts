 ...
 */
const checkFileSize = (actualSize: number, maxSize: number | null, minSize: number = 0, file?: File): ValidationErrors | null => {
    return (!IsNullOrEmpty(maxSize) && actualSize > maxSize!) || actualSize < minSize ?
        {maxSize, minSize, actual: actualSize, file} : null;
};

const getFileType = (file: File, fileExtension: string | undefined): FileUploadTypes => {
...
export class FileUploadValidators {
     ...
     * @dynamic
     */
    public static fileSize(maxSize: number): ValidatorFn | null  {
        return (control: AbstractControl | FileUploadControl): {fileSize: Array<ValidationErrors>} |  any  => {
            const files: Array<File> = control.value;
            if (IsNullOrEmpty(files)) { return null!; }
            checkValueType(files);
     ...
     * @dynamic
     */
    public static sizeRange({ minSize, maxSize }: { minSize?: number | null; maxSize?: number | null }): ValidatorFn {
        return (control: AbstractControl | FileUploadControl): {sizeRange: Array<ValidationErrors>} | any => {
            const files: Array<File> = control.value;
            if (IsNullOrEmpty(files)) { return null!; }
            checkValueType(files);
     ...
     * @dynamic
     */
    public static filesLimit(numFiles: number | undefined): ValidatorFn {
        return (control: AbstractControl | FileUploadControl): ValidationErrors | any => {
            const files: Array<File> = control.value;
            if (IsNullOrEmpty(files)) { return null!; }
            checkValueType(files);
