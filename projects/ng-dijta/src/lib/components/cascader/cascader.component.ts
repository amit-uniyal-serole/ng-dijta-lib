...
import { DropDownAppendToBodyComponent, DropDownDirective } from '../dropdown';
...
export class CascaderComponent implements OnInit, OnDestroy, OnChanges, AfterContentInit, ControlValueAccessor {
  ...
  ngAfterContentInit(): void {
    if (!this.width) {
      this.width = this.el.nativeElement.parentElement.clientWidth - 40;
  ...
  get listOfOptions() {
    return this.multipleValueList.filter((val) => val?.isRoot)
  }
  ...
  getMultipleValueFromValueList(valueList: Array<number | string>[]): CascaderItem[] {
    ...
    valueList.forEach(value => {
      ...
      let cacheTarget: CascaderItem | any;
      ...
      for (let i = 0; i < value.length; i++) {
        ...
        if (this.isLazyLoad && cur && !cur.children?.length && !cur.isLeaf) {
          this.cascaderSrv.lazyloadMultipleChild(cur, i, true);
          break;
        } else {
  ...
  getLabelFromValue(value: Array<number | string | undefined>): string {
    let cur;
    let list = this.cascaderSrv.options;
    value.forEach(item => {
  ...
  getPathLabelFormValue(value: Array<number | string | undefined>): string {
    let path = '';
    let cur;
    let list = this.cascaderSrv.options;
  ...
  onToggle(isOpen: boolean): void {
    ...
    if (!isOpen && this.allowSearch) {
      if (!this.cascaderSrv.currentValue.length) {
        ...
        this.writeValue(this.cascaderSrv.currentValue as any);
      }
