...
export type COPY_MODE_TYPE = 'block' | 'address' | 'select' | undefined;
...
export class NgxIpBase implements OnChanges, ControlValueAccessor, Validator {
  ...
  public addr: AddressModeLogic | undefined;
  private _focused;
  public get focused() {
  ...
  set separator(value: string) {
    ...
    this.separatorMap = this.addr ? this.addr.blocks().map( b => value) : [];
    if(this.addr) {
  ...
  @ViewChildren('input', { read: ElementRef }) public inputs: QueryList<ElementRef> | undefined;
  ...
  private autoCopy: 'DEFAULT_BLOCK' | 'DEFAULT_ADDRESS' | COPY_METHOD | 'IN_FLIGHT' | undefined;
  ...
  ngOnChanges(changes: SimpleChanges) {
    if ('separator' in changes) {
      if (!changes.separator.currentValue) {
        this.separator = this.addr ? this.addr.SEP : '';
  ...
  onCopy($event: ClipboardEvent, idx: number): void {
    ...
    try {
      switch (this.autoCopy) {
        case 'block':
        case 'DEFAULT_BLOCK':
          this.autoCopy = this.autoCopy === 'block' ? undefined : this.autoCopy;
          return;
        case 'address':
        case 'DEFAULT_ADDRESS':
          this.autoCopy = this.autoCopy === 'address' ? undefined : this.autoCopy;
          const value = this.fromBlocks(this.blocks);
          if($event.clipboardData && value) {
  ...
  onKeyPress($event: KeyboardEvent, idx: number): void {
    ...
    if (this.inputValidation === 'char' && this.addr && !this.addr.RE_CHAR.test(key)) {
      return cancelEvent($event);
    } else if (this.inputValidation === 'block' && this.addr && !this.addr.RE_BLOCK[idx].test(value)) {
      return cancelEvent($event);
    }

    this.markBlockValidity(value, idx);
    if (!this.invalidBlocks[idx] && isLast && this.addr && this.addr.isMaxLen(value)) {
  ...
  onKeyUp($event: KeyboardEvent, idx: number): void {
    if (this.isBackspace($event)) {
      ...
      const value = input && input.selectionStart && input.selectionStart >= 0 && input.selectionEnd && input.selectionEnd > input.selectionStart
  ...
  protected paste(data: string, blockIndex: number): boolean {
    let arr = this.addr ? this.addr.split(data, this.separator) : [];
    if (this.addr && arr.length === this.addr.BLOCK_COUNT) {
      this.value = this.fromBlocks(arr);
    } else {
      let val = this.getInputElement(blockIndex);
      if(val) {
        const value = inputSelection.insert(val, arr[0]);
  ...
  private copyUserSelectedMethod(blockIndex: number) {
    let htmlValue = this.getInputElement(blockIndex);
    if(htmlValue) {
      try {
        const input: HTMLInputElement = htmlValue;
